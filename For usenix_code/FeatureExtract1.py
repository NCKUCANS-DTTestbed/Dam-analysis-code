...
with open('Dataset_1.csv', 'w',) as csvfile:
    ...
    with open('exploit_data.json', 'r',encoding="utf-8") as f:
...
with open('Dataset_2.csv', 'w') as csvfile:
    ...
    writer.writerow(['No', 'Trela','Protocol','IP_src','IP_dst','Tdelta','Function1','Function2','Function3',
                     'Function4','Function5','Function6','Trela1','Trela_fun','Function-gram','TFfun','Answer'])
    with open('normal_data.json', 'r' ,encoding="utf-8") as f:
        Json_data = json.load(f)
...
thresh = data2.loc[((data2['IP_src'] == '192.168.1.100') | (data2['IP_src'] == '192.168.1.101')
                   | (data2['IP_src'] == '192.168.1.102') | (data2['IP_src'] == '192.168.1.103')
                   | (data2['IP_src'] == '192.168.1.104') | (data2['IP_src'] == '192.168.1.105')
                   | (data2['IP_src'] == '192.168.1.106') | (data2['IP_src'] == '0')) &
                   ((data2['IP_dst'] == '192.168.1.100') | (data2['IP_dst'] == '192.168.1.101')
                    | (data2['IP_dst'] == '192.168.1.102') | (data2['IP_dst'] == '192.168.1.103')
                    | (data2['IP_dst'] == '192.168.1.104') | (data2['IP_dst'] == '192.168.1.105')
                    | (data2['IP_dst'] == '192.168.1.106') | (data2['IP_dst'] == '0'))
                   ]
...
for k in pro_list:
    for i in ip_list:
        for j in ip_list:
            ...
            test_DF = data2.loc[(((data2['IP_src'] == i) & (data2['IP_dst'] == j)) | ((data2['IP_src'] == j)
                                                                                         & (data2['IP_dst'] == i)))
                                                                                            & (data2.Protocol == k)]
            test_DF['Function2'] = test_DF['Function1'].shift(1)
            test_DF['Function3'] = test_DF['Function1'].shift(2)
            ...
            test_DF['Function5'] = test_DF['Function1'].shift(4)
            test_DF['Function6'] = test_DF['Function1'].shift(5)
            thresh_DF = pd.concat([thresh_DF, test_DF ],axis=0, ignore_index=True)

            #data.Function1[(data.IP_src == i) | (data.IP_dst == j) & (data.Protocol == k)] = data['Function'].shift(1)
            #data.Function2[(data.IP_src == i) | (data.IP_dst == j) & (data.Protocol == k)] = data['Function'].shift(2)

thresh_DF = thresh_DF.drop_duplicates()
thresh_DF = thresh_DF.sort_values('No')
thresh_DF['Function-gram'] = thresh_DF['Function1']*10000000000\
                             +thresh_DF['Function2']*100000000\
                             +thresh_DF['Function3']*1000000\
                             +thresh_DF['Function4']*10000\
                             +thresh_DF['Function5']*100\
                             +thresh_DF['Function6']
print(thresh_DF.shape)
print("------------------------------------------------------------------------------------------------")
result_DF = pd.concat([result_DF, thresh_DF ],axis=0, ignore_index=True)
